...
void		patch_bis(t_info *info, int32_t nb)
{
	...
	start += 5;
	if (nb == 1)
		start -= 0x2b2;//REPLACE1
	if (nb == 2)
		start -= 0x1ed;//REPLACE2
	if (nb == 3)
		start -= 0x1e8;//REPLACE3
	if (nb == 4)
		start -= 0x2ab;//REPLACE4
	if (nb == 5)
		start -= 0x156;//REPLACE5
	end = (int32_t)((size_t)(info->addr_hooked_func) - (size_t)(info->text_begin) + info->text_addr);
	val = end - start;
	if (nb == 1)
		ft_memcpy(info->file + info->offset_bis + BIS_SIZE - 0x2b2/*REPLACE1*/ + 1, &val, 4);
	if (nb == 2)
		ft_memcpy(info->file + info->offset_bis + BIS_SIZE - 0x1ed/*REPLACE2*/ + 1, &val, 4);
	if (nb == 3)
		ft_memcpy(info->file + info->offset_bis + BIS_SIZE - 0x1e8/*REPLACE3*/ + 1, &val, 4);
	if (nb == 4)
		ft_memcpy(info->file + info->offset_bis + BIS_SIZE - 0x2ab/*REPLACE4*/ + 1, &val, 4);
	if (nb == 5)
		ft_memcpy(info->file + info->offset_bis + BIS_SIZE - 0x156/*REPLACE5*/ + 1, &val, 4);
}
